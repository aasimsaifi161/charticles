...
export function DashboardHeader({
  ...
  const [isMobileChartTypeOpen, setIsMobileChartTypeOpen] = useState(false);
  ...
  useEffect(() => {
    const handleClickOutside = (e: MouseEvent) => {
      ...
      if (mobileTypeRef.current && !mobileTypeRef.current.contains(e.target as Node)) {
        setIsMobileChartTypeOpen(false);
      }
    };
  ...
  const activeTypeItem = chartTypes.find((t) => t.id === activeChartType) || chartTypes[0];
  const ActiveIcon = activeTypeItem.icon;

  return (
    <header className="w-full h-14 shrink-0 flex items-center justify-between border-b border-white/[0.08] bg-[#09090B] px-3 sm:px-4 backdrop-blur-md z-40 select-none">
      {/* Left: Brand & Title */}
      <div className="flex items-center gap-2 sm:gap-3 min-w-0">
        <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-xl border border-[#2ff0d6]/30 bg-black/80 shadow-md shadow-[#2ff0d6]/15 group transition-all">
        ...
        <div className="flex flex-col min-w-0">
          <div className="flex items-center gap-1.5 sm:gap-2">
            ...
            <span className="hidden xs:inline-block rounded-full bg-[#2ff0d6]/10 border border-[#2ff0d6]/30 px-1.5 py-0.2 text-[9px] font-mono font-medium text-[#2ff0d6] shadow-sm shadow-[#2ff0d6]/10">
              Free
            </span>
          </div>
          ...
          <div className="flex items-center gap-1 mt-0.5 min-w-0">
            {isEditing ? (
              <input
                ...
                className="bg-black/80 border border-[#2ff0d6]/50 rounded px-1.5 py-0.5 text-[9px] font-mono text-white outline-none w-28 sm:w-36 shadow-sm"
              />
            ) : (
              <button
                type="button"
                onClick={() => setIsEditing(true)}
                className="group flex items-center gap-1 rounded bg-white/[0.04] border border-white/10 px-1.5 py-0.5 text-[9px] font-mono text-[#8E97A8] hover:text-white hover:border-[#2ff0d6]/40 hover:bg-white/[0.08] transition-all cursor-pointer truncate max-w-[100px] sm:max-w-[140px]"
      ...
      {/* Center: Desktop Chart Types & Presets */}
      ...
      {/* Mobile Chart Type Dropdown (Shown only on < md) */}
      <div className="flex md:hidden items-center gap-1.5 relative" ref={mobileTypeRef}>
        <button
          type="button"
          onClick={() => setIsMobileChartTypeOpen(!isMobileChartTypeOpen)}
          className="inline-flex items-center gap-1.5 rounded-lg border border-white/10 bg-white/[0.04] px-2.5 py-1 text-xs font-medium text-white hover:bg-white/[0.08] transition-all shadow-sm cursor-pointer"
        >
          <ActiveIcon className="h-3.5 w-3.5 text-[#2ff0d6]" />
          <span className="font-semibold text-[11px] text-[#2ff0d6]">{activeTypeItem.label}</span>
          <ChevronDown className={`h-3 w-3 text-[#8E97A8] transition-transform ${isMobileChartTypeOpen ? "rotate-180" : ""}`} />
        </button>

        {isMobileChartTypeOpen && (
          <div className="absolute top-full left-1/2 -translate-x-1/2 mt-1.5 w-56 rounded-xl border border-white/10 bg-[#0D0F14] p-1.5 shadow-2xl backdrop-blur-xl z-50 animate-in fade-in zoom-in-95 duration-150">
            <div className="px-2 py-1 text-[10px] font-mono uppercase tracking-wider text-[#8E97A8] border-b border-white/[0.06] mb-1">
              Select Chart Type
            </div>
            <div className="space-y-0.5">
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center justify-between px-2.5 py-1.5 rounded-lg text-xs transition-colors cursor-pointer ${
                      isSelected
                        ? "bg-[#2ff0d6]/15 text-[#2ff0d6] font-semibold"
                        : "text-[#cad2e0] hover:bg-white/[0.06] hover:text-white"
                    }`}
                  >
                    <div className="flex items-center gap-2">
                      <Icon className={`h-3.5 w-3.5 ${isSelected ? "text-[#2ff0d6]" : "text-[#8E97A8]"}`} />
                      <span>{type.label}</span>
                    </div>
                    {isSelected && <Check className="h-3.5 w-3.5 text-[#2ff0d6]" />}
                  </button>
                );
              })}
            </div>
      ...
      <div className="flex items-center gap-1.5 sm:gap-2.5 shrink-0 justify-end">
        ...
        <button
          type="button"
          onClick={onReset}
          className="inline-flex items-center gap-1 rounded-lg border border-white/10 bg-white/[0.03] px-2 sm:px-2.5 py-1 text-xs font-medium text-[#8E97A8] hover:text-white hover:bg-white/[0.06] transition-all cursor-pointer"
        ...
        <button
          ...
          className="inline-flex items-center gap-1.5 rounded-lg bg-[#2ff0d6] px-2.5 sm:px-3.5 py-1 text-xs font-semibold text-[#06070a] shadow-sm shadow-[#2ff0d6]/20 hover:bg-[#28d7bf] hover:shadow-[#2ff0d6]/30 active:scale-[0.98] transition-all cursor-pointer"
